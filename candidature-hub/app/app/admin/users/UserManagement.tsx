...
export default function UserManagement() {
  ...
  return <>
    ...
    <div className="grid gap-3 sm:grid-cols-3">
      <div className="surface-card p-4"><p className="text-sm text-slate-500">Utenti totali</p><p className="mt-1 text-3xl font-bold">{users.length}</p></div>
      <div className="surface-card p-4"><p className="text-sm text-slate-500">Accessi attivi</p><p className="mt-1 text-3xl font-bold text-[#5f6f58]">{activeCount}</p></div>
      <div className="surface-card p-4"><p className="text-sm text-slate-500">Amministratori</p><p className="mt-1 text-3xl font-bold text-[#9d4f39]">{users.filter(user => user.isActive && user.role === "ADMIN").length}</p></div>
    </div>
    ...
    <section className="surface-card p-5 sm:p-6">
      ...
      <form onSubmit={createUser} className="mt-5 grid gap-4 md:grid-cols-2">
        ...
        <div className="md:col-span-2 flex justify-end"><button disabled={busy === "create"} className="primary-action min-h-12 rounded-lg px-6 font-semibold disabled:opacity-50">{busy === "create" ? "Creazione…" : "Crea utente"}</button></div>
      </form>
    </section>
    ...
    <section className="space-y-3">
      ...
      {loading && <div className="surface-card p-6 text-sm text-slate-500">Caricamento utenti…</div>}
      {!loading && users.map(user => <article key={user.id} className={`surface-card p-4 sm:p-5 ${user.isActive ? "" : "opacity-70"}`}>
        {editing?.id === user.id ? <form onSubmit={updateUser} className="grid gap-4 md:grid-cols-2">
          ...
          <div className="flex justify-end gap-2 md:col-span-2"><button type="button" onClick={() => setEditing(null)} className="secondary-action min-h-11 rounded-lg px-4 font-semibold">Annulla</button><button disabled={busy === user.id} className="primary-action min-h-11 rounded-lg px-5 font-semibold">Salva modifiche</button></div>
